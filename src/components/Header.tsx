    ...
    <header className='w-full flex justify-center min-h-24'>
      <section className='w-[85%] flex flex-row justify-between items-center'>
        ...
        <nav
          ...
          } transition-all duration-500 fixed left-0 bottom-0 w-full rounded-t-xl bg-white px-2 py-2 shadow-lg lg:static lg:my-0 lg:flex lg:w-auto lg:rounded-full`}
        >
          <ul className='flex flex-col justify-center items-center w-full lg:flex-row lg:items-center lg:justify-between gap-2'>
            {Sections.map((section) => (
              <li
                onClick={handleClick}
                className={`transition-all duration-300 w-full lg:w-auto rounded-xl lg:rounded-full bg-[var(--${section.color})] lg:bg-transparent text-base hover:bg-[var(--${section.color})] hover:font-medium hover:text-white cursor-pointer px-4 py-2 text-center`}
              >
